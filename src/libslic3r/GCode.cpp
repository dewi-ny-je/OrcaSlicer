...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    // Orca: user option to omit informational/structural comment blocks from the exported G-code
    const bool omit_gcode_header_blocks = print.config().omit_gcode_header_blocks.value;

    if(!has_BTT_thumbnail && !omit_gcode_header_blocks){
    ...
    // Orca: skipped together with the header block when omit_gcode_header_blocks is enabled.
    if (!omit_gcode_header_blocks) {
